...
### Phase 5 — ลงประกาศให้เช่า (List Item) — วางแผนไว้ ยังไม่ทำ

> อ้างอิงสเปกจาก [`requirement.md`](./requirement.md) หัวข้อ 4.6 "ลงประกาศสินค้า" — หน้าฝั่ง**ผู้ใช้ทั่วไป** (marketplace ไม่ใช่ admin) สำหรับสร้าง/แก้ไข/พัก/ลบประกาศให้เช่าสินค้าของตัวเอง ต่อยอดจาก `Category`/`Product`/`ProductImage`/`PickupOption` ที่เพิ่งเพิ่มกลับเข้า schema ใน Phase 4.6 — ตอนนี้มี model พร้อมแล้ว เหลือแค่สร้าง API สำหรับ **owner (user ทั่วไป)** และหน้าเว็บฝั่ง marketplace
>
> **หมายเหตุ:** Phase 4.8 (Admin สินค้า) ตอนนี้ยังว่างเปล่าเพราะไม่มีใครสร้างสินค้าได้เลย — Phase นี้คือสิ่งที่จะทำให้ตาราง Admin > สินค้า เริ่มมีข้อมูลจริง

- [ ] 5.1 API: `POST /api/products` — สร้างประกาศ (auth, ทุก user ที่ login แล้วสร้างได้ ไม่ต้องเป็น role พิเศษ)
  ...
  - 📝 commit: `feat(api): create product listing endpoint`

- [ ] 5.2 API: `PUT /api/products/:id` (owner เท่านั้น) + `PATCH /api/products/:id/status` (owner: pause/resume เท่านั้น สลับระหว่าง `ACTIVE`↔`PAUSED` — ห้าม set `UNDER_REVIEW`/`ACTIVE` เองถ้ายังไม่เคยผ่านการอนุมัติ) + `DELETE /api/products/:id` (soft delete ตาม Dev Standard #9)
  - 🧪 test: แก้ไข/ลบประกาศของคนอื่น → 403 | pause แล้ว resume ได้เฉพาะประกาศที่เคย `ACTIVE` มาก่อน | ลบแล้วหายจาก `GET /api/me/listings`
  - 📝 commit: `feat(api): update, pause/resume, and delete product listing`

- [ ] 5.3 API: อัปโหลดรูปสินค้า + จัดการจุดรับสินค้า
  - `POST /api/products/:id/images` (owner, multer, รองรับหลายรูป, เรียงตาม `sortOrder`) — ใช้ `createImageUpload()` เดิมจาก Phase 3 (แค่เปลี่ยน subdir เป็น `products`)
  - `POST /api/products/:id/pickup-options` + `DELETE /api/products/:id/pickup-options/:optionId` (owner)
  - 🧪 test: อัปโหลดไม่ใช่รูป → 400 (ใช้ fileFilter เดิม) | อัปโหลด/ลบรูปของสินค้าคนอื่น → 403 | เพิ่ม/ลบจุดรับสินค้าได้
  - 📝 commit: `feat(api): product images and pickup options`

- [ ] 5.4 API: `GET /api/me/listings` (auth) — ประกาศทั้งหมดของ user ที่ login (ทุกสถานะ รวม `UNDER_REVIEW`/`PAUSED`)
  - 🧪 test: คืนเฉพาะประกาศของ user นั้น ไม่เห็นของคนอื่น
  - 📝 commit: `feat(api): my listings endpoint`

- [ ] 5.5 Web: หน้าลงประกาศ (`modules/products/components/ListItemForm.tsx`, route `app/(marketplace)/list-item/page.tsx`)
  - ตามสเปก requirement.md: อัปโหลดรูป (เริ่ม 1 รูปก่อน ขยายเป็นหลายรูปทีหลังได้), ชื่อสินค้า, หมวดหมู่ (dropdown จาก `GET /api/categories` — active only ตาม Dev Standard #12), ราคา/วัน (บาท), รายละเอียด (textarea)
  - จุดรับสินค้า: chip ที่เลือก/ลบได้ + ช่องเพิ่มจุดใหม่ (เหมือน pattern เดิมใน prototype)
  - ที่ตั้งสินค้า: ช่องกรอกที่อยู่แบบข้อความก่อน (แผนที่จริงเป็น Phase ถัดไปตาม requirement.md — ต้องเลือก Maps provider ก่อน)
  - ปุ่มส่งประกาศ → เรียก `POST /api/products` จริง (ไม่ใช่แค่ตั้ง flag เหมือน prototype เดิม) แล้วอัปโหลดรูป/pickup options ตามลำดับ → หน้าสำเร็จ "ส่งประกาศสำเร็จ! รอตรวจสอบ" + ปุ่ม "ลงประกาศเพิ่ม"
  - 🧪 test: กรอกครบ + อัปโหลดรูป → เรียก API สำเร็จ → เห็นประกาศจริงใน `GET /api/me/listings` และใน Admin > สินค้า ด้วย (status รอตรวจสอบ)
  - 📝 commit: `feat(web): list item form matching requirement.md spec`

- [ ] 5.6 Web: หน้า My Listings (`modules/products/components/MyListingsTable.tsx`, route `app/(marketplace)/my-listings/page.tsx`)
  - รายการประกาศของฉัน + ปุ่มพัก/เริ่มใหม่ (เฉพาะประกาศที่ผ่านการอนุมัติแล้ว) + ปุ่มแก้ไข (เปิด `ListItemForm` เดิมในโหมดแก้ไข) + ปุ่มลบ
  - 🧪 test: พัก/เริ่มใหม่/ลบ ทำงานถูกต้อง | แก้ไขประกาศแล้วข้อมูลอัปเดตจริง
  - 📝 commit: `feat(web): my listings page`

- [ ] 5.7 Auto test: ครอบคลุม create → upload image/pickup options → pause/resume → edit → delete ครบวงจร + ownership 403 ทุก endpoint
  - 🧪 test: `npm test --workspace=apps/api` → ผ่านทุก case
  - 📝 commit: `test(api): product listing lifecycle tests`
