...
// เนื้อหาล้วนๆ ของหน้ารายละเอียดสินค้า (gallery/ข้อมูล/สินค้าคล้ายกัน) — ไม่มี container/back-link
// ของตัวเอง เพราะใช้ร่วมกันทั้งแบบเต็มหน้า (ProductDetailPage) และแบบ modal (ProductDetailModal)
    ...
    <div>
      <div className="grid grid-cols-1 gap-8 md:grid-cols-2">
        ...
        <div>
          ...
          <button
            type="button"
            // ตั้งใจ hard-navigate (ไม่ใช้ router.push) — ถ้าหน้านี้กำลังแสดงเป็น modal
            // (intercepting route) อยู่ router.push แบบ soft navigation จะไม่ล้าง @modal slot
            // เพราะ Next.js คงค่า parallel slot เดิมไว้ถ้า path ใหม่ไม่ตรงกับ slot นั้น (ตามที่
            // เอกสาร Next.js ระบุไว้) ทำให้ backdrop ของ modal เดิมค้างอยู่หลังไปหน้า checkout แล้ว
            onClick={() => {
